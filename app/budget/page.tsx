    ...
    if (user?.companyId) {
      if (session.role === "employee") {
        ...
        if (teamIds.length > 0) {
          ...
          entries = await prisma.budgetEntry.findMany({
            where: { projectId: { in: projects.map((p) => p.id) } },
            orderBy: { date: "desc" },
          });
        }
      } else {
        // Admins and managers: show all projects and entries from their company
    ...
    <div className="p-8 animate-slide-up">
      ...
      <div className="grid grid-cols-3 gap-4 mb-8">
        ...
        <div className="glass rounded-xl p-5">
          ...
          <p
            className={`font-display text-3xl font-bold ${remaining < 0 ? "text-rose" : "text-jade"}`}
          >
          ...
          <div className="p-6 space-y-4">
            {byProject.map((p) => (
              <div key={p.id}>
                ...
                <p
                  className={`text-xs mt-1 ${p.pct > 80 ? "text-rose" : "text-white/30"}`}
                >
                  {p.pct}% used
                </p>
              </div>
            ...
            {Object.entries(byCategory)
              ...
              .map(([cat, amount]) => (
                <div key={cat} className="flex items-center justify-between">
                  <span
                    className={`text-xs px-2.5 py-1 rounded-full font-medium ${CATEGORY_COLORS[cat] || "bg-white/10 text-white/60"}`}
                  >
                    {cat}
                  </span>
                  <div className="flex items-center gap-3">
                    <div className="w-24 h-1.5 bg-white/10 rounded-full overflow-hidden">
                      <div
                        ...
                        style={{
                          width: `${Math.round((amount / totalSpent) * 100)}%`,
                        }}
                      />
                    </div>
        ...
        <table className="w-full">
          <thead>
            <tr className="border-b border-white/5">
              {["Date", "Project", "Description", "Category", "Amount"].map(
                (h) => (
                  <th
                    key={h}
              ...
              return (
                ...
                >
                  ...
                  <td className="px-6 py-3">
                    <span
                      className={`text-xs px-2 py-0.5 rounded-full ${CATEGORY_COLORS[entry.category] || "bg-white/10 text-white/50"}`}
                    >
                      {entry.category}
                    </span>
                  </td>
                  <td className="px-6 py-3 text-sm font-medium text-rose">
