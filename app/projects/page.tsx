  ...
  if (isEmployee && session) {
    ...
    if (!session) {
      ...
      if (!user?.companyId) {
        ...
        projects = await (prisma.project.findMany as any)({
          where:
            teamIds.length > 0 ? { teamId: { in: teamIds } } : { teamId: null },
          include: { tasks: true, team: { select: { name: true } } },
          orderBy: { createdAt: "desc" },
        });
      }
      ...
      {isEmployee && projects.length === 0 && (
        <div className="glass rounded-xl p-12 text-center border border-white/5">
          ...
          <p className="text-white/50 text-sm font-medium">
            You&apos;re not assigned to any team yet.
          </p>
          <p className="text-white/25 text-xs mt-1">
            Contact your manager to be added to a team and project.
          </p>
        </div>
      )}
        ...
        {projects.map((p: any) => {
          ...
          return (
            <Link key={p.id} href={`/projects/${p.id}`}>
              <div
                className={`glass rounded-xl p-6 border hover:border-accent/30 transition-all cursor-pointer ${riskColor} ${riskBg}`}
              >
                <div className="flex items-start justify-between mb-4">
                  <div>
                    <div className="flex items-center gap-2 mb-1">
                      ...
                      {p.riskScore >= 70 && (
                        <span className="text-xs bg-rose/20 text-rose px-2 py-0.5 rounded-full border border-rose/20">
                          ⚠ High Risk
                        </span>
                      )}
                    </div>
                  ...
                  <div className="text-right">
                    <p
                      className={`text-sm font-medium ${daysLeft <= 7 ? "text-rose" : daysLeft <= 14 ? "text-amber" : "text-white/60"}`}
                    >
                ...
                <div className="flex items-center justify-between">
                  <div className="flex gap-4 text-xs text-white/40">
                    ...
                    {blocked > 0 && (
                      <span className="text-rose">✕ {blocked} blocked</span>
                    )}
                  </div>
                  <div className="text-right">
                    ...
                    <span
                      className={`text-xs ml-1 ${budgetPct > 80 ? "text-rose" : "text-white/30"}`}
                    >
                      ({budgetPct}%)
                    </span>
                  </div>
