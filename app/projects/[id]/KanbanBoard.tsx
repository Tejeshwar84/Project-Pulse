...
export default function KanbanBoard({
  ...
  function formatDeadline(dueDate: string | null): {
    text: string;
    isOverdue: boolean;
  } {
  ...
  const sortedVisibleTasks =
    sortByPriority && prioritizationData.length > 0
        ...
        <div className="px-6 py-4 border-b border-white/5 space-y-3 bg-gradient-to-r from-amber/5 to-rose/5 animate-in fade-in">
          <div className="flex items-center justify-between">
            ...
            {prioritizationData.length > 0 && (
              <label className="flex items-center gap-2 cursor-pointer text-[11px]">
                ...
                />
                <span className="text-white/60 hover:text-white/80">
                  Sort by priority
                </span>
              </label>
            )}
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-5 gap-2">
            {topPriorityTasks.map((item, idx) => {
              ...
              const isFirst = idx === 0;
              const hasBothHighs =
                taskRisk && taskRisk.riskScore > 70 && item.priorityScore > 70;
              ...
              return (
                ...
                >
                  <div className="flex items-start justify-between gap-2 mb-2">
                    <p
                      className={`text-[11px] font-medium flex-1 leading-snug ${isFirst ? "text-amber" : "text-white/80"}`}
                    >
                      {task.title}
                    </p>
                  ...
                  <div className="flex items-center justify-between gap-2">
                    <span
                      className={`text-[10px] font-semibold ${getPriorityColor(item.priorityScore)}`}
                    >
      ...
      {workloadData && (
        <div className="px-6 py-4 border-b border-white/5 space-y-4 bg-surface-2/50 animate-in fade-in">
          <div className="flex items-center justify-between gap-3">
            <div>
              <h3 className="text-sm font-semibold text-jade flex items-center gap-2">
                ⚖️ Workload Optimization
              </h3>
              <p className="text-[10px] text-white/40">
                Team capacity and suggested task reassignments.
              </p>
            </div>
            <span className="text-[10px] text-white/50">
              {workloadData.users.length} team members
            </span>
          </div>
            ...
            <div className="rounded-xl border border-white/10 bg-white/5 p-3">
              ...
              <div className="space-y-2">
                {workloadData.users.map((user) => (
                  <div
                    key={user.userId}
                    className="flex items-center justify-between gap-3 rounded-lg border border-white/10 bg-surface-3 p-3"
                  >
                    <div>
                      ...
                      <p className="text-[9px] text-white/40">
                        Tasks: {user.taskCount} • High: {user.highPriorityCount}
                      </p>
                    </div>
                    <span
                      className={`text-[10px] font-semibold ${user.status === "overloaded" ? "text-red-400" : user.status === "underutilized" ? "text-emerald-400" : "text-amber-400"}`}
                    >
                      {user.status === "overloaded"
                        ? "🔴 overloaded"
                        : user.status === "underutilized"
                          ? "🟢 underutilized"
                          : "🟡 balanced"}
                    </span>
                  </div>
            ...
            <div className="rounded-xl border border-white/10 bg-white/5 p-3">
              ...
              <div className="space-y-2">
                {workloadData.suggestions.length > 0 ? (
                  workloadData.suggestions.map((suggestion) => {
                    const task = tasks.find((t) => t.id === suggestion.taskId);
                    const fromUser = workloadData.users.find(
                      (user) => user.userId === suggestion.fromUserId,
                    );
                    const toUser = workloadData.users.find(
                      (user) => user.userId === suggestion.toUserId,
                    );
                    return (
                      <div
                        key={`${suggestion.taskId}-${suggestion.toUserId}`}
                        className="rounded-lg border border-white/10 bg-surface-3 p-3"
                      >
                        <p className="text-[11px] font-semibold text-white">
                          Task: {task?.title || suggestion.taskId}
                        </p>
                        <p className="text-[9px] text-white/40">
                          From: {fromUser?.userName || suggestion.fromUserId}
                        </p>
                        <p className="text-[9px] text-white/40">
                          To: {toUser?.userName || suggestion.toUserId}
                        </p>
                  ...
                  <p className="text-[10px] text-white/40">
                    No reassignment suggestions available.
                  </p>
                )}
              </div>
            </div>
      ...
      {adding && isAdmin && (
        <div className="px-6 py-4 border-b border-white/5 space-y-3 bg-surface-2/50">
          ...
          <div className="flex gap-2 flex-wrap">
            ...
            <button
              onClick={addTask}
              className="px-3 py-1.5 bg-accent text-white text-xs rounded-lg hover:bg-accent/80 transition-colors"
            >
              Add
            </button>
            <button
              onClick={() => setAdding(false)}
              className="px-3 py-1.5 text-white/40 text-xs hover:text-white transition-colors"
            >
              Cancel
            </button>
          </div>
        ...
        {COLUMNS.map((col) => {
          ...
          return (
            <div key={col.id} className="p-3 min-w-[160px]">
              <div
                className={`flex items-center justify-between mb-3 px-2 py-1.5 rounded-lg ${col.headerBg}`}
              >
                <span
                  className={`text-[10px] font-bold uppercase tracking-wider ${col.color}`}
                >
                  {col.label}
                </span>
              ...
              <div className="space-y-2">
                {colTasks.map((task) => {
                  ...
                  const hasBothHighs =
                    taskRisk &&
                    taskRisk.riskScore > 70 &&
                    taskPriority &&
                    taskPriority.priorityScore > 70;

                  return (
                    <div
                      key={task.id}
                      className={`bg-surface-3 rounded-lg p-3 border-l-2 ${PRIORITY_COLOR[task.priority]} group transition-all ${
                        isTopPrio
                          ? "ring-1 ring-amber/50 shadow-lg shadow-amber/20"
                          : ""
                      } ${hasBothHighs ? "ring-1 ring-red-400/50" : ""}`}
                    >
                      <div className="flex items-start justify-between gap-2">
                        <p className="text-xs text-white/80 group-hover:text-white mb-2 leading-snug flex-1">
                          {task.title}
                        </p>
                        {isTopPrio && (
                      ...
                      {assigneeName && (
                        <div className="flex items-center gap-1.5 mb-2">
                          <div
                            className={`w-4 h-4 rounded-full flex items-center justify-center text-[9px] font-bold ${isMyTask ? "bg-jade/30 text-jade" : "bg-accent/30 text-accent-light"}`}
                          >
                            {assigneeName
                              .split(" ")
                              .map((n: string) => n[0])
                      ...
                      {deadlineInfo.text && (
                        <div
                          className={`text-[9px] mb-2 ${deadlineInfo.isOverdue ? "text-red-400 font-medium" : "text-white/40"}`}
                        >
                          {deadlineInfo.text}
                        </div>
                      )}
                      ...
                      {taskPriority && (
                        <div className="mb-2">
                          <div
                            className={`text-[9px] font-medium ${getPriorityColor(taskPriority.priorityScore)}`}
                          >
                            {getPriorityBadge(taskPriority.priorityScore)} •{" "}
                      ...
                      {taskRisk && (
                        <div className="mb-2">
                          <div
                            className={`text-[9px] font-medium ${getRiskColor(taskRisk.riskScore)}`}
                          >
                        ...
                        {!(isAdmin && isPendingReview) &&
                          moves.map((c) => (
                            <button
                              ...
                              className={`text-[9px] transition-colors border rounded px-1 py-0.5 ${
                                c.id === "pending-review"
