...
export default async function Dashboard() {
  ...
  const doneTasks = projects.reduce(
    (s, p) => s + p.tasks.filter((t: any) => t.status === "done").length,
    0,
  );
  ...
  const stats = [
    ...
    },
    {
      ...
      sub: `${formatCurrency(totalSpent, "USD")} spent`,
      color: "text-sky",
    },
    {
      label: "Tasks Complete",
      value: `${doneTasks}/${totalTasks}`,
      sub: `${totalTasks > 0 ? Math.round((doneTasks / totalTasks) * 100) : 0}% done`,
      color: "text-jade",
    },
    {
      label: "Budget Health",
  ...
  return (
    <div className="p-8 animate-slide-up">
        ...
        <div className="mb-6 bg-rose/10 border border-rose/20 rounded-xl px-5 py-4 flex items-start gap-3">
          ...
          <div>
            <p className="text-rose font-medium text-sm font-display">
              {atRisk} project{atRisk > 1 ? "s" : ""} need
              {atRisk === 1 ? "s" : ""} attention
            </p>
      ...
      {/* Admin Approvals */}
      {session?.role === "admin" && <AdminApprovals />}
              ...
              return (
                ...
                >
                  <td className="px-6 py-4">
                    <Link
                      href={`/projects/${p.id}`}
                      className="font-medium text-white/90 group-hover:text-white text-sm transition-colors"
                    >
                      {p.name}
                    </Link>
                  ...
                  <td className="px-6 py-4">
                    <div>
                      ...
                      <p className="text-xs text-white/30">
                        of ${p.budget.toLocaleString()} ({budgetPct}%)
                      </p>
                    </div>
                  </td>
                  <td className="px-6 py-4">
                    <p
                      className={`text-sm ${daysLeft <= 7 ? "text-rose" : daysLeft <= 14 ? "text-amber" : "text-white/60"}`}
                    >
                      {daysLeft > 0
                        ? `${daysLeft}d left`
