...
export async function POST(req: Request) {
  ...
  const highPrioOpen = tasks.filter(
  ...
  const factors = {
    deadline: clamp(
      daysLeft <= 0 ? 100 : ((30 - Math.min(daysLeft, 30)) / 30) * 100,
    ),
...
Provide a direct, practical risk assessment. Identify the top risks and suggest 1-2 concrete actions the PM should take now.`;
  ...
  try {
    ...
    if (daysLeft < 10 && completionPct < 70)
      risks.push(
        `only ${daysLeft} days remain with ${completionPct}% completion`,
      );
    if (blocked > 0) {
      risks.push(`${blocked} task${blocked > 1 ? "s are" : " is"} blocked`);
      actions.push("resolve blockers immediately");
    }
    if (highPrioOpen > 2) {
      risks.push(`${highPrioOpen} high-priority tasks are still open`);
      actions.push("prioritize high-priority tasks");
    }
    if (budgetPct > 80) actions.push("review remaining budget allocation");
    if (daysLeft < 14 && todo > 2)
      actions.push(`consider deferring ${todo} non-critical tasks`);
