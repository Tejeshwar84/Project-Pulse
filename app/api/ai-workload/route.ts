...
function buildFallbackSuggestions(
  ...
  const underutilizedUsers = users.filter(
    (user) => user.status === "underutilized",
  );
  ...
  const sortedUnderutilized = [...underutilizedUsers].sort(
    (a, b) => a.taskCount - b.taskCount,
  );

  for (const overloaded of overloadedUsers) {
    const candidateTasks = (tasksByUser.get(overloaded.userId) || []).sort(
      (a, b) => {
...
export async function POST(req: Request) {
  try {
    ...
    const allUsers = teamUsers.reduce<Record<string, ProjectUser>>(
      (acc, user) => {
...
Avoid any extra text or formatting.`;
    ...
    try {
      ...
      const validatedUsers = parsed.users.map(
        (item: any) =>
          ({
            userId: item.userId,
            userName: item.userName,
            taskCount: Number.isFinite(item.taskCount)
              ? item.taskCount
              : parseInt(item.taskCount) || 0,
            highPriorityCount: Number.isFinite(item.highPriorityCount)
              ? item.highPriorityCount
              : parseInt(item.highPriorityCount) || 0,
            status:
              item.status === "overloaded" ||
              item.status === "balanced" ||
              item.status === "underutilized"
                ? item.status
                : getFallbackStatus(item.taskCount, item.highPriorityCount),
          }) as WorkloadUser,
      );

      const validatedSuggestions = parsed.suggestions.map(
        (item: any) =>
          ({
            taskId: item.taskId,
    ...
    return NextResponse.json(
      { error: "Failed to optimize workload" },
      { status: 500 },
    );
  }
}
