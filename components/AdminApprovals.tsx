...
export default function AdminApprovals() {
    ...
    return (
      <div className="mb-6 bg-amber/10 border border-amber/20 rounded-xl p-5">
        <div className="flex items-center justify-between mb-4">
          <h3 className="font-display font-semibold text-amber">Pending Approvals</h3>
  ...
  return (
    <div className="mb-6 bg-amber/10 border border-amber/20 rounded-xl p-5">
      <div className="flex items-center justify-between mb-4">
        <h3 className="font-display font-semibold text-amber">Pending Approvals</h3>
        <span className="text-xs text-amber/60">{pendingUsers.length} user{pendingUsers.length > 1 ? 's' : ''} awaiting approval</span>
      </div>
        ...
        {pendingUsers.map((user) => (
          <div key={user.id} className="flex items-center justify-between p-3 bg-white/5 rounded-lg">
            <div>
              ...
              <p className="text-white/60 text-xs">{user.email} • Requested {user.requestedRole}</p>
            </div>
